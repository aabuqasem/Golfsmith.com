...
class Order {
  ...
  public function processOrder() {
    ...
    if(empty($v_process)) {
      ...
      if($v_success === TRUE && empty($v_return_status)) {
        ...
        gsi_mssql_bind($v_stmt, "@p_profile_scope", &$v_profile_scope, 'varchar', 30);
        gsi_mssql_bind($v_stmt, "@p_return_status", &$v_process_status, 'varchar', 30, true);
  ...
  private function recordQBDSold($p_qbd_id, $p_qbd_sold) {
  	...
  	$web_db1 = mysql_pconnect(MS_DB_SERVER_WRITE, MS_DB_ADMINUSER, MS_DB_ADMINPASSWORD) ;
  	   
    $v_sql = "INSERT into webonly.gsi_qbd_sold values ($p_qbd_id,now(),$p_qbd_sold)"; 

    $v_result = mysql_query($v_sql, $web_db1);
  ...
  private function decreaseQBDQty($p_qbd_id, $p_qty) {
    ...
    while ($myrow = mysql_fetch_array($v_result)) {
  ...
  public function repriceOrder() {
    ...
    if($_SESSION['s_do_reprice'] == 'Y') {
      ...
      gsi_mssql_bind($v_stmt, "@p_original_system_reference", &$this->v_order_number, 'varchar', 30);
      gsi_mssql_bind($v_stmt, "@p_profile_scope", &$v_profile_scope, 'varchar', 30);
      gsi_mssql_bind($v_stmt, "@p_return_status", &$v_return_status, 'varchar', 50, true);
  ...
  public function addCreditCardPayment($p_cc_num, $p_expiry_month, $p_expiry_year, $p_deferment, $p_save_cc_info = 'N',$p_pin_number) {
    ...
    if(!empty($p_cc_num)) {
      ...
      $v_status  = $i_payment->add_payment($v_net_total, $p_cc_num, $v_expiry, $v_is_golfsmith_cc, $p_save_cc_info,$p_pin_number) ;
...
//    }
  ...
  private function translateShipMethod($p_ship_method) {
    ...
    switch(strToUpper($p_ship_method)) {
      case 'FEDEX-GND-BUSINESS':
      case 'FEDEX-GND-RESIDENTIAL':
        $v_result = 'FedEx Ground';
        break;
      case 'FEDEX-2DAY-ACC':
      case 'FEDEX-2DAY':
        $v_result = 'FedEx 2-Day';
        break;
      case 'FEDEX-STD-OVERNIGHT':
      case 'FEDEX-NEXTDAY':
        $v_result = 'UPS Next Day';
        break;
      case 'UPS-GROUND-ACC':
        $v_result = 'UPS Ground';
        break;
      ...
      case 'UPS-CANADA-PP':
        ...
        break;
      case 'UPS-CAN-EXPEDITED':
        $v_result = 'UPS Expedited';
        break;
      case 'UPS-CAN-EXPRESS':
        $v_result = 'UPS Express';
        break;
      case 'UPS-CAN-STD-ZN1':
        $v_result = 'UPS Ground';
        break;
