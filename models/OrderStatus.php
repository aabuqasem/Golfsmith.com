...
class OrderStatus{
	...
	{   		
        ...
        if (!empty($order->OrigSysDocumentRef)){        
        		$results["resultStatus"]= "ok";
        		$results["headers"][$headercounter]["orderNumber"]= $order->OrigSysDocumentRef;
        ...
        } else 	{ $results["resultStatus"]= "empty"; }
	...
	{   
		...
		if (!empty($header->OrigSysDocumentRef)){
        		$results["resultStatus"]= "ok";
        		$results["headers"][$headercounter]["orderNumber"]= $header->OrigSysDocumentRef;
            	...
            	if($header->shipDate=="N/A")
          		...
          		$results["headers"][$headercounter]["TrackingNumber"] = $header->TrackingNumber;
	...
	{	
        ...
        if (sizeof($result->orders->Order->header) > 0) {
        	...
        	if (!empty($header->OrigSysDocumentRef)){        
        		$results["resultStatus"]= "ok";
            	$results["orderNumber"] = $header->OrigSysDocumentRef;
          		$results["ShipMethodCode"] = $header->ShippingMethodCode;
				$results["FullName"] = $header->CustomerAccount->Party->PersonFirstName." ".$header->CustomerAccount->Party->PersonFirstName;    	          									
                ...
                //if (!is_array($orders->lines->line)) {
                //      $orders->lines->line = array($orders->lines->line);
               // }				
